...
- 🔒 **余额不足阻止**：负向应用余额不足时显示阻止页面，3 秒后自动跳转桌面
- 📱 **实时监控**：后台服务实时监控应用使用情况（0.1 秒检测间隔）
- 🔔 **使用时长提醒**：负向应用使用时定期显示横幅通知提醒（完全不阻挡操作）
...
**新增功能**
- 🔔 使用时长提醒：负向应用使用时显示系统横幅通知
- ✅ 权限自动刷新：授权后立即更新 UI 状态，无需重启

**优化改进**
- 🏠 拦截后自动跳转桌面：余额不足时阻止页面3秒后自动返回桌面
- ⏱️ 冷却时间优化：拦截冷却时间延长至 30 秒，避免频繁弹窗
- 📱 兼容性增强：通知方案兼容所有厂商（包括三星 One UI）
- 🔄 应用列表自动加载：授权后自动检测权限并刷新应用列表
